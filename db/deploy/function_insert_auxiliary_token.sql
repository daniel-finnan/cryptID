-- Deploy cryptID:function_insert_auxiliary_token to pg

BEGIN;

CREATE OR REPLACE FUNCTION dtif.insert_auxiliary_token (json_input JSONB)
	RETURNS void
	LANGUAGE plpgsql
AS $$
BEGIN
	...
	--Normative
	INSERT INTO dtif.auxiliary_token (auxiliary_dti, auxiliary_mechanism, auxiliary_distributed_ledger, auxiliary_technical_reference)
		SELECT
			(json_record -> 'Header' ->> 'DTI')::VARCHAR(9) AS auxiliary_dti,
			(json_record -> 'Normative' ->> 'AuxiliaryMechanism')::dtif.AUXILIARY_MECHANISM AS auxiliary_mechanism,
			(json_record -> 'Normative' ->> 'AuxiliaryDistributedLedger')::VARCHAR(9) AS auxiliary_distributed_ledger,
			(json_record -> 'Normative' ->> 'AuxiliaryTechnicalReference')::TEXT AS auxiliary_technical_reference
		FROM dtif.staging_json;
